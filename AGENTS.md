...
Desktop meeting recorder. Session 1 is local audio capture on Linux. Do not deploy. Do not delete `docs/`.
...
- Linux first. Windows and macOS capture are stubs until someone on that OS implements them.
...
A recording is not done until Start writes a file and Stop leaves a playable WAV on disk.
