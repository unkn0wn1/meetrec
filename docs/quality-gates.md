...
4. `npm run test` — Vitest. Pure helpers only (paths, duration, Pulse parse, Windows device lists, session status, folder layout, WAV migration, transcript segments, minutes markdown, provider selection, xAI device-code parsing, secret-bag codec, ffmpeg binary resolution). No Electron window required.
