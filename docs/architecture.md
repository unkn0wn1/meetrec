...
| `search`     | qmd CLI, export markdown, `search.json`, model cache under userData                         |
...
- `recordings/<id>/search.json` — optional. Written when meeting search indexes that recording (`pending`, `indexed`, or `error`). A missing file means the recording is not in the index.
- `userData/qmd-export/meetings/<id>.md` — transcript markdown (and the summary, when `summary.md` is non-empty) for the qmd collection `meetings`.
- Models and the qmd index live under `userData/qmd-home`. The index is the transcript and the summary when one exists. `audio.mp3` is not an input.
