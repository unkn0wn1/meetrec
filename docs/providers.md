...
- Before Transcribe uploads, main runs ffmpeg to mono 16 kHz MP3 (`libmp3lame`, 48 kbps). Library playback still uses the original stereo 48 kHz `audio.wav`. That keeps uploads under Cloudflare’s request-body caps on `api.x.ai` and under OpenAI’s ~25 MB file limit for typical meeting lengths.
