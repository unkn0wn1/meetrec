...
Test classifies ids as a best effort. Voice ids contain `transcribe` or `whisper` (including `grok-voice-transcribe`) and do not contain `realtime` or `tts`. OpenAI chat ids start with `gpt-`, `o` plus a digit, or `chatgpt-`. xAI chat ids start with `grok-`. Embedding, moderation, image, audio, video, TTS, and realtime ids stay out of chat. If nothing matches either role, both selects get the full id list. A role that matches nothing while the other role matches stays empty. A seed is added to its role only when that exact id was in the response.
...
A fresh install defaults both roles to `xai-key` with the xAI seeds above. xAI `GET /v1/models` may omit the speech models. Voice then stays empty and Transcribe keeps `grok-voice-transcribe-2.0`. Device-code tokens are not documented for that route; a rejection leaves the xAI sign-in selects empty. Meetrec does not invent a list.
